// Script test Admin login với tài khoản đã tồn tại
// Chạy script này trong browser console

const testAdminExisting = async () => {
  try {
    console.log("🔍 === TESTING ADMIN LOGIN WITH EXISTING ACCOUNTS ===");

    // Danh sách tài khoản Admin đã tồn tại
    const adminAccounts = [
    ...
    console.log("\n🔍 === ADMIN EXISTING TEST COMPLETED ===");
  } catch (error) {
    console.error("❌ Error in admin existing test:", error);
  }
};

// Export function
window.testAdminExisting = testAdminExisting;
