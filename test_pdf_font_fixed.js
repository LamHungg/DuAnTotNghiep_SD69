// Script test PDF với font Unicode đã được sửa
// Copy và paste vào Console của browser khi đang ở trang Statistics

const testPDFFontFixed = () => {
  console.log("🔍 Test PDF Với Font Unicode Đã Được Sửa...\n");
  ...
  try {
    ...
    // Test 2: Kiểm tra dữ liệu gửi đến PDF API với font Unicode đã sửa
    console.log("\n2️⃣ Test Dữ Liệu Gửi Đến PDF API Với Font Unicode Đã Sửa:");

    const testPDFFontData = async () => {
      try {
        ...
        console.log(
          "✅ PDF Payload với dữ liệu thực tế và font Unicode đã sửa:",
          pdfPayload
        );
        ...
        if (pdfResponse.ok) {
          ...
          if (contentType && contentType.includes("application/pdf")) {
            ...
            console.log(
              "💡 PDF sẽ chứa dữ liệu thực tế và font Unicode đã được sửa"
            );
          } else {
        ...
        console.log("❌ Lỗi test PDF API font đã sửa:", error.message);
      }
    };
    ...
    console.log("\n3️⃣ Test Button Export PDF Với Font Unicode Đã Sửa:");
    ...
    // Test 4: Kiểm tra font Unicode đã sửa
    console.log("\n4️⃣ Test Font Unicode Đã Được Sửa:");
    ...
    const fontTest = () => {
      ...
      console.log("\n✅ Font Unicode đã được sửa sẽ hỗ trợ:");
      console.log("   - Sử dụng Times Roman với IDENTITY_H encoding");
      console.log("   - Fallback to Helvetica với IDENTITY_H encoding");
    ...
    console.log(
      "\n✅ Test hoàn thành! PDF sử dụng dữ liệu thực tế và font Unicode đã được sửa"
    );
  } catch (error) {
    console.log("❌ Lỗi test PDF font đã sửa:", error.message);
  }
};

// Test backend endpoints với font Unicode đã sửa
const testBackendFontFixed = () => {
  console.log("\n🔍 Test Backend Endpoints Với Font Unicode Đã Sửa...\n");
  ...
  try {
    ...
    console.log("❌ Lỗi test backend font đã sửa:", error.message);
  }
};

// Chạy tests
testPDFFontFixed();
testBackendFontFixed();

// Export functions
window.testPDFFontFixed = testPDFFontFixed;
window.testBackendFontFixed = testBackendFontFixed;
