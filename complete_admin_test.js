// Script test hoàn chỉnh cho hệ thống Admin
// Chạy script này trong browser console khi đang ở trang Admin

const completeAdminTest = async () => {
  try {
    ...
    if (response.ok) {
      ...
      console.log("3. Comprehensive data analysis:");
    ...
    console.log("=== COMPLETE ADMIN SYSTEM TEST COMPLETED ===");
    console.log("🎊 Hệ thống quản lý đơn hàng Admin đã sẵn sàng!");
  } catch (error) {
    console.error("❌ Error in complete admin test:", error);
  }
};

// Export function
window.completeAdminTest = completeAdminTest;
