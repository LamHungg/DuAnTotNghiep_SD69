...
async function testFrontendExactFlow() {
  try {
    console.log("🧪 Test luồng frontend chính xác...\n");

    // 1. Đăng nhập để tạo session (giống như frontend)
    console.log("1️⃣ Đăng nhập để tạo session...");
    ...
    // 3. Mô phỏng dữ liệu từ localStorage (giống như frontend)
    console.log("\n3️⃣ Mô phỏng dữ liệu từ localStorage...");
    ...
    // 4. Tạo đơn hàng với session authentication (giống như frontend)
    console.log("\n4️⃣ Tạo đơn hàng với session authentication...");
    const orderData = {
      khachHangId: user.id,
      diaChiId: 1,
      loaiDonHang: true,
    ...
    if (
      ...
      console.log("   Luồng frontend hoạt động bình thường.");
      console.log("   Vấn đề có thể ở frontend UI hoặc localStorage.");
    } else {
      console.log("❌ FAILED: Số lượng sản phẩm không được trừ!");
      console.log("   Có vấn đề với luồng backend.");
    }
  } catch (error) {
