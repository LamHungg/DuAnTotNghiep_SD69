console.log(`
🔧 TEST PROFILE FIX FINAL - Copy và paste vào Console:
  ...
  if (response.status === 200) {
    ...
    if (Array.isArray(data)) {
      ...
      if (data.length > 0) {
        ...
        console.log('- Trạng thái:', order.trangThaiDonHang?.tenTrangThai || order.trangThai);
        ...
        if (order.chiTietDonHang && order.chiTietDonHang.length > 0) {
          ...
          console.log('- Tên sản phẩm:', item.chiTietSanPham?.sanPham?.tenSanPham || item.tenSanPham);
          console.log('- Số lượng:', item.soLuong);
          console.log('- Đơn giá:', item.donGia || item.gia);
        }
      }
    } else {
